...
func handleChannels(chans <-chan ssh.NewChannel, server *Server) {
	for newChannel := range chans {
		go handleChannel(newChannel, server)
	}
}
...
func handleChannel(ch ssh.NewChannel, server *Server) {
	...
	remoteSession, err := NewSshSession(server.Host, server.Port, server.User, server.Pass)
	if err != nil {
		log.Printf("remote session err %v", err)
		connection.Close()
		return
	}
	// connDemo := NewChannelDemocrator(connection)
...
func main() {
	...
	for {
		...
		// 接收所有channels
		go handleChannels(chans, GlobalConfig.GetServer(sshConn.User()))
	}
}
